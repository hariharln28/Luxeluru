    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-[#9a8fa8]">Smart navigation — salons ranked by proximity</p>
        </div>
      ...
      <div className="mt-8 space-y-3">
        {ranked.map(({ item, distance }, idx) => (
          <div key={item.id} className="luxe-card flex flex-col gap-4 p-4 sm:flex-row sm:items-center justify-between">
            <div className="flex items-center gap-4 min-w-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="flex items-center gap-1 text-xs text-[#9a8fa8] truncate">
                  <MapPin className="h-3 w-3 shrink-0" /> {item.area} · {item.address}
                </p>
                <div className="mt-1 flex items-center gap-3">
                  <span className="flex items-center gap-1 text-xs sm:text-sm">
                    <Star className="h-3.5 w-3.5 fill-[#c9a962] text-[#c9a962]" /> {item.rating}
                  </span>
                  <span className="text-xs sm:text-sm font-medium text-[#c9a962]">
